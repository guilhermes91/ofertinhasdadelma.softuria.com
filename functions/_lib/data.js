...
// URL-fonte → dedup automático (mesma foto-fonte = mesma key = 1 objeto no R2). Path ÚNICO
// (flat, sem prefixo de loja — o hash já é globalmente único). O upload roda num endpoint
// SEPARADO (/api/mirror, cron próprio), nunca no caminho do bot.
const IMG_EXT = { "image/jpeg": "jpg", "image/jpg": "jpg", "image/png": "png", "image/webp": "webp", "image/gif": "gif", "image/avif": "avif" };

export async function mirrorKey(srcUrl, contentType = "") {
  ...
  return `${hex.slice(0, 40)}.${ext}`;
}

// Já é uma imagem hospedada por nós? (qualquer path /img/) → não re-busca da fonte.
export function isHostedImage(url) {
  return /\/img\//.test(String(url || ""));
}
